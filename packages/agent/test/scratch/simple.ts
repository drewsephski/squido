...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".squido/skills")),
		source("user", join(homedir(), ".squido/agent/skills")),
...
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[source("project", join(env.cwd, ".squido/prompts")), source("user", join(homedir(), ".squido/agent/prompts"))],
