# @drewsepsi/squido-agent-core

Stateful agent with tool execution and event streaming. Built on `@drewsepsi/squido-ai`.

## Installation

```bash
npm install @drewsepsi/squido-agent-core
```
...
```typescript
declare module "@drewsepsi/squido-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@drewsepsi/squido-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@drewsepsi/squido-agent-core";
