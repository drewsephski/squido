import { Markdown, type MarkdownTheme } from "@drewsepsi/squido-tui";
...
import { getLatestSquidoRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
Options:
  -l, --local       Install project-locally (.squido/settings.json)
...
Options:
  -l, --local       Remove from project settings (.squido/settings.json)
  -a, --approve     Trust project-local files for this command
  -na, --no-approve Ignore project-local files for this command
...
Update squido and installed packages.

Options:
  --self                  Update squido only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall squido even if the current version is latest
...
function printSelfUpdateUnavailable(npmCommand?: string[], updatePackageName = PACKAGE_NAME): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of squido executable: ${entrypoint}`);
	}
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestSquidoRelease(VERSION);
