...
import { type ImageContent, modelsAreEqual } from "@drewsepsi/squido-ai";
...
import { ENV_SESSION_DIR, expandTildePath, getAgentDir, getPackageDir, getWebUiDir, VERSION } from "./config.ts";
...
import { handleReviewCommand } from "./review-command.ts";
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.SQUIDO_OFFLINE);
	if (offlineMode) {
		process.env.SQUIDO_OFFLINE = "1";
	...
	if (await handleReviewCommand(args)) {
		return;
	}
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.SQUIDO_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: SQUIDO_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
	...
	if (appMode === "rpc") {
		...
		await runWebMode(runtime, {
			staticDir: parsed.webUiDir ?? getWebUiDir(),
			port: parsed.port,
			host: parsed.host,
			openBrowser: true,
		});
