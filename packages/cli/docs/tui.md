> squido can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@drewsepsi/squido-tui`](https://github.com/drewsephski/squido/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@drewsepsi/squido-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `SQUIDO_HARDWARE_CURSOR=1` (also `PI_HARDWARE_CURSOR=1`). The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@drewsepsi/squido-tui";
...
Import from `@drewsepsi/squido-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@drewsepsi/squido-tui";
```
...
```typescript
import { matchesKey, Key } from "@drewsepsi/squido-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@drewsepsi/squido-tui";
...
```typescript
import { getMarkdownTheme } from "@drewsepsi/squido-cli";
import { Markdown } from "@drewsepsi/squido-tui";
...
Set `SQUIDO_TUI_WRITE_LOG` (or the legacy `PI_TUI_WRITE_LOG`) to capture the raw ANSI stream written to stdout.

```bash
SQUIDO_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@drewsepsi/squido-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@drewsepsi/squido-cli";
import { DynamicBorder } from "@drewsepsi/squido-cli";
import { Container, type SelectItem, SelectList, Text } from "@drewsepsi/squido-tui";
...
```typescript
import { BorderedLoader } from "@drewsepsi/squido-cli";
...
For toggling multiple settings. Use `SettingsList` from `@drewsepsi/squido-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@drewsepsi/squido-cli";
import { Container, type SettingItem, SettingsList, Text } from "@drewsepsi/squido-tui";
...
Customize the inline working indicator shown while squido is streaming a response.
...
// Restore squido's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@drewsepsi/squido-tui";
