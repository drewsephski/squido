> squido can create themes. Ask it to build one for your setup.
...
Squido loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.squido/agent/themes/*.json`
- Project: `.squido/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `squido.themes` entries in `package.json`
...
On first run, squido detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.squido/agent/themes
vim ~/.squido/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/drewsephski/squido/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, squido reloads it automatically for immediate visual feedback.
...
Squido uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, squido falls back to the nearest approximation.
