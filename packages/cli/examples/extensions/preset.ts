 ...
 * - ~/.squido/agent/presets.json (global)
 * - <cwd>/.squido/presets.json (project-local)
 *
...
import type { ExtensionAPI, ExtensionContext } from "@drewsepsi/squido-cli";
import { DynamicBorder, getAgentDir } from "@drewsepsi/squido-cli";
import { Container, Key, type SelectItem, SelectList, Text } from "@drewsepsi/squido-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".squido", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.squido/agent/presets.json or .squido/presets.json", "warning");
			return;
		}
