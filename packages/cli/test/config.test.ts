...
const originalPiPackageDir = process.env.SQUIDO_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalPiPackageDir === undefined) {
		delete process.env.SQUIDO_PACKAGE_DIR;
	} else {
		process.env.SQUIDO_PACKAGE_DIR = originalPiPackageDir;
	}
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	tempDir = prefix;
	process.env.SQUIDO_PACKAGE_DIR = packageDir;
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@drewsepsi/squido-cli")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @drewsepsi/squido-cli",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@drewsepsi/squido-cli")).toBeUndefined();
		expect(getUpdateInstruction("@drewsepsi/squido-cli")).toBe(
			"Update @drewsepsi/squido-cli using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@drewsepsi/squido-cli");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@drewsepsi/squido-cli",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @drewsepsi/squido-cli`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@drewsepsi/squido-cli", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @drewsepsi/squido-cli`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@drewsepsi/squido-cli")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @drewsepsi/squido-cli",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@drewsepsi/squido-cli"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @drewsepsi/squido-cli",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@drewsepsi/squido-cli";
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.SQUIDO_PACKAGE_DIR = storePackageDir;
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@drewsepsi/squido-cli")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@drewsepsi/squido-cli")).toContain(
