 ...
 *
 * Runs a simple tool loop directly against the squido-ai provider source so it does not
...
import { AuthStorage } from "../../cli/src/core/auth-storage.ts";
...
function parseArgs(argv: string[]): Args {
	...
	let sessionId = `squido-ai-codex-ws-cached-probe-${Date.now()}`;
