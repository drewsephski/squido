 ...
 */
export const CURSOR_MARKER = "\x1b_squido:c\x07";
...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.SQUIDO_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.SQUIDO_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.SQUIDO_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".squido", "agent", "squido-debug.log");
		...
		// Configurable via setClearOnShrink() or SQUIDO_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".squido", "agent", "squido-crash.log");
				const crashData = [
		...
		if (process.env.SQUIDO_TUI_DEBUG === "1") {
